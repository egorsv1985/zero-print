...
<section class="py-12 works bg-primary rounded-standard">
	<div class="container">
		...
		<div class="grid grid-cols-12 mb-5 space-x-8 space-y-8 lg:mb-0">
			<? foreach ($arResult["ITEMS"] as $key => $arItem): ?>
				...
				?>
				<div id="<?= $this->GetEditAreaId($arItem['ID']); ?>" class=" col-span-12 xl:col-span-4 <?= ($key === 2) ? 'lg:row-span-3' : 'lg:col-span-6'; ?> ">
					...
					<picture>
						<source srcset="<?= $arItem["PREVIEW_PICTURE"]["WEBP"] ?>" type="image/webp"><img src="<?= $arItem["PREVIEW_PICTURE"]["PNG"] ?>" alt="<?= $arItem["PREVIEW_PICTURE"]["ALT"] ?>"
							title="<?= $arItem["PREVIEW_PICTURE"]["TITLE"] ?>" class="w-full h-full" width="410" height="250">
					</picture>
				</div>
		</div>
	<? endforeach; ?>
